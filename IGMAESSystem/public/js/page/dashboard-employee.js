...
$(function () {
  ...
  function loadproductiontable() {
    $.ajax({
      type: 'get',
      url: '/admin/dashboard/productionemployee',
      dataType: 'json',
  ...
  async function fethProfitData() {
    return new Promise((resolve, reject)=>{
      $.ajax({
        type: 'get',
        url: '/admin/dashboard/individualemployee',
        dataType: 'json',
        success: function (response) {
          resolve(response);
        },
  ...
  async function initProfitTable(){
    ...
    console.log(data);
    const ctx_production = document.getElementById('salary_graph');
    const config = {
      type: 'line',
      data: data,
      options: {
